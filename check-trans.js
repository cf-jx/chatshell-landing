...
const zhMatch = js.match(/'zh-CN': \{([\s\S]*?)\},\s*'en'/);
if (zhMatch) {
...
const enSection = js.match(/'en': \{([\s\S]*?)\}\};/);
if (enSection) {
    const enText = enSection[1];
...
const zhSection = js.match(/'zh-CN': \{([\s\S]*?)\},\s*'en'/);
if (zhSection) {
    const zhText = zhSection[1];
    ...
    const techTerms = ['ChatGPT', 'ChatShell', 'Markdown', 'PDF', 'DOCX', 'HTML', 'JSON', 'LaTeX', 'Pro', 'Standard', 'Free', 'Chrome', 'Edge', 'Brave', 'Arc', 'License', 'Key', 'Creem'];
